...
export class PensionerEmployeeDataService {
  ...
  async findByValuatedEmployeesProjectAndStage(projectId: string, stageName: string): Promise<{ ECODE: number; NAME: string }[]> {
    const filter: any = { project: projectId };
    ...
    return this.pensionerEmployeeDataModel.find(filter).select('ECODE NAME -_id').exec();
  }
